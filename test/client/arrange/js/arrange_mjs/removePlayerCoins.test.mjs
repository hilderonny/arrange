...
describe('arrange.mjs removePlayerCoins()', () => {
    ...
    it('Es wird die API GET /api/player/removecoins/test_user_id/12345 aufgerufen.', async () => {
        ...
        global.fetch = (url) => {
            assert.ok(url.endsWith('/api/player/removecoins/test_user_id/12345'))
            fetchWasCalled = true
            return { ok: true, json() { return {} } }
        }
        await arrange.removePlayerCoins(12345)
    ...
    it('Bei Erfolg wird der Status als JSON-Objekt zurückgegeben.', async () => {
        ...
        const result = await arrange.removePlayerCoins(12345)
