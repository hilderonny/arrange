...
async function publishRoutes(arrange) {
    ...
    // Alle APIs erstellen
    createListApi(arrange, 'users/users', '/api/users/listusers', [ 'USERS_ADMINISTRATION_USER' ])
    createListApi(arrange, 'users/usergroups', '/api/users/listusergroups', [ 'USERS_ADMINISTRATION_USER' ])
    createListApi(arrange, 'users/permissions', '/api/users/listpermissions', [ 'USERS_ADMINISTRATION_USER', 'HOME_APPMANAGEMENT' ])
    createSaveApi(arrange, 'users/permissions', '/api/users/savepermission', [ 'USERS_ADMINISTRATION_USER' ])
    createSaveApi(arrange, 'users/users', '/api/users/saveuser', [ 'USERS_ADMINISTRATION_USER' ])
    createSaveApi(arrange, 'users/usergroups', '/api/users/saveusergroup', [ 'USERS_ADMINISTRATION_USER' ])
    createDeleteApi(arrange, 'users/users', '/api/users/deleteuser', [ 'USERS_ADMINISTRATION_USER' ])
    createDeleteApi(arrange, 'users/usergroups', '/api/users/deleteusergroup', [ 'USERS_ADMINISTRATION_USER' ])
    await loadApis(arrange, './modules/users/api')
}
